...
function fetchRooms(roomType, page) {
    $.ajax({
        type: "get",
        async: true,
        url: 'http://14.36.141.71:9994:10079/GDJ79_main_semi/reservation/date',
        dataType: "json",
        data: {
            checkindate: a[0],
            checkoutdate: a[1],
        ...
        success: function (response) {
            ...
            //console.log(e.target.previousElementSibling.previousElementSibling.previousElementSibling.previousElementSibling.lastElementChild.value); //자차부분                 
             ...
             location.assign(`http://14.36.141.71:9994:10079/GDJ79_main_semi/pay/paymentPage?roomNo=${roomList[i].roomNo}&checkindate=${a[0]}&checkoutdate=${a[1]}&Mrequest=${request}&bedType=${bedType}&car=${car}&peopelNo=${roomPeopleNo.substring(0,roomPeopleNo.length-1)}&price=${diff*Number(room.roomPrice)}`);
                });
                // detail div에 요소 추가
